    ...
    if word != '':
        ...
        if slash == '':
            ...
            if word == 'endstream':
                ...
                insideStream = False
        if fOut != None:
            if slash == '/' and '/' + word in ('/JS', '/JavaScript', '/AA', '/OpenAction', '/JBIG2Decode', '/RichMedia', '/Launch', '/Names', '/FlateDecode'):
...
class cPDFiD():
    def __init__(self, xmlDoc, force):
        ...
        # New Tags
        self.title = self.keywords['/Title']
        self.names = self.keywords['/Names']
        self.font = self.keywords['/Font']
        self.flatedecode = self.keywords['/FlateDecode']
